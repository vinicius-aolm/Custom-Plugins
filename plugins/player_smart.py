...
@userge.on_message(main_filter & cmd_filter('play'))
...
@userge.on_message(main_filter & cmd_filter('volume'))
...
@userge.on_message(main_filter & cmd_filter('join'))
@init_client_and_delete_message
async def start(_, message: Message):
    await group_call.start(message.chat.id, False)


@userge.on_message(main_filter & cmd_filter('leave'))
@init_client_and_delete_message
async def stop(*_):
    await group_call.stop()


@userge.on_message(main_filter & cmd_filter('rejoin'))
@init_client_and_delete_message
async def reconnect(*_):
    await group_call.reconnect()


@userge.on_message(main_filter & cmd_filter('replay'))
@init_client_and_delete_message
async def restart_playout(*_):
    group_call.restart_playout()


@userge.on_message(main_filter & cmd_filter('stop'))
@init_client_and_delete_message
async def stop_playout(*_):
    group_call.stop_playout()


@userge.on_message(main_filter & cmd_filter('mute'))
@init_client_and_delete_message
async def mute(*_):
    group_call.set_is_mute(True)


@userge.on_message(main_filter & cmd_filter('unmute'))
@init_client_and_delete_message
async def unmute(*_):
